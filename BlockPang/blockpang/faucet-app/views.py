...
def req_icx(request,to_address):
    ...
    page = 'block height : '+str(int(block_height,16))+'<div></div>'+' find_transaction : '+str(int(find_transaction,16))+'<div>tx_hash : </div>'+tx_hash + '<div></div>'+'block:'+'<div></div>'+get_address +' // block balance'+ str(int(get_balance,16)/10**18) +'<div>to:</div>'+ str(wallet_to)+' // balance is '+ str(int(get_to,16)/10**18)
    return HttpResponse(page)